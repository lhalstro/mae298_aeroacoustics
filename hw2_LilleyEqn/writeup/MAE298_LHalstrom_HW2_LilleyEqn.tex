...
Next, a single expression achieving both of the required constraints can be obtained by substituting Eqn~\ref{VrMomentum} applied to the inner and outer solutions:
...
\dfrac{dP}{dr}^+ &= \dfrac{d}{dr} \left[C H_n^{(1)}(\lambda_{\infty} r) \right]
    = C \dfrac{dH_n^{(1)}(\lambda_{\infty} r)}{dr} \\
\dfrac{dP}{dr}^- &= \dfrac{d}{dr} \left[A J_n(\lambda_j r) \right]
    = A \dfrac{dJ_n(\lambda_j r)}{dr}
\end{align*}

\noindent and substitute the resulting expressions into Eqn~\ref{constraints}:

\begin{align*}
\dfrac{C \dfrac{dH_n^{(1)}(\lambda_{\infty} r)}{dr}}{(\omega - kW_{\infty})^2}
    = \dfrac{A \dfrac{dJ_n(\lambda_j r)}{dr}}{(\omega - kW_{j})^2} \\
\dfrac{A}{C} = \dfrac{(\omega - kW_{j})^2}{(\omega - kW_{\infty})^2}
  \dfrac{ \dfrac{dH_n^{(1)}(\lambda_{\infty} r)}{dr}}{\dfrac{dJ_n(\lambda_j r)}{dr}}
\end{align*}
