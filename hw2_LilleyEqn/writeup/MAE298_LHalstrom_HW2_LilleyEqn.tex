...
For the first constraint, we will derive a relationship for the pressure fluctuation using the momentum equation:
...
\noindent where $V_r$ is the radial component of velocity at the vortex sheet:
...
Beginning with the second constraint, the radial velocity fluctuation $V_r$ can be defined as the time derivative of the radial displacement $\zeta$ of fluid particles located at the vortex sheet.
...
\begin{align}
V_r^+ &= -a ( i\omega - kW_{\infty} ) \exp(-i\omega t + kz)  \label{Vplus}\\
V_r^- &= -a ( i\omega - kW_j ) \exp(-i\omega t + kz) \label{Vminus}
\end{align}

Solving the solution for $V_r$ for the constant terms, and setting Eqns~\ref{Vplus} and~\ref{Vminus} equal to each other:

\begin{align}
-a \exp(-i\omega t + kz) = \dfrac{V_r^+}{i\omega - kW_{\infty}}
